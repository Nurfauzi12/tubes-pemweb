  ...
  <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl " id="navbarBlur" data-scroll="false">
    <div class="container-fluid py-1 px-3">
      <nav aria-label="breadcrumb">
        ...
        <h3 class="font-weight-bolder text-white mb-0">Edit Nilai Kompetensi</h3>
      </nav>
    </div>
  </nav>
      ...
      <div class="col-12">
        <div class="card mb-4">
          <div class="card-header pb-0">
            <h5>Form Edit Nilai Kompetensi</h5>
            <p class="text-sm mb-0">Perbarui data nilai kompetensi mahasiswa</p>
          </div>
            ...
            <form action="" method="post" id="formNilai">
                  ...
                  <div class="input-group mb-3">
                    <select name="nim" class="form-control <?= isset($validation) && $validation->hasError('nim') ? 'is-invalid' : '' ?>" required>
                      ...
                      <?php foreach ($mahasiswa ?? [] as $m): ?>
                        <option value="<?= esc($m['nim']) ?>" <?= $nilai['nim'] == $m['nim'] ? 'selected' : '' ?>><?= esc($m['nim']) ?> - <?= esc($m['nama_mahasiswa']) ?></option>
                      <?php endforeach; ?>
                    </select>
                  ...
                  <div class="input-group mb-3">
                    <select name="id_rencana_pembelajaran" class="form-control <?= isset($validation) && $validation->hasError('id_rencana_pembelajaran') ? 'is-invalid' : '' ?>" required>
                      ...
                      <?php foreach ($rencana ?? [] as $r): ?>
                        <option value="<?= esc($r['id']) ?>" <?= $nilai['id_rencana_pembelajaran'] == $r['id'] ? 'selected' : '' ?>><?= esc($r['minggu_ke']) ?> - <?= esc($r['materi']) ?></option>
                      <?php endforeach; ?>
                    </select>
              ...
              <div class="row">
                <div class="col-md-4">
                  ...
                  <div class="input-group mb-3">
                    <input type="number" name="nilai_kompetensi" min="0" max="100" class="form-control <?= isset($validation) && $validation->hasError('nilai_kompetensi') ? 'is-invalid' : '' ?>" value="<?= old('nilai_kompetensi', $nilai['nilai_kompetensi']) ?>" required>
                ...
                <div class="col-md-4">
                  ...
                  <div class="input-group mb-3">
                    <select name="status" class="form-control <?= isset($validation) && $validation->hasError('status') ? 'is-invalid' : '' ?>" required>
                      ...
                      <option value="belum" <?= $nilai['status']=='belum' ? 'selected' : '' ?>>Belum</option>
                    </select>
                ...
                <div class="col-md-4">
                  ...
                  <div class="input-group mb-3">
                    <input type="text" name="keterangan" class="form-control" value="<?= old('keterangan', $nilai['keterangan']) ?>">
                  </div>
                </div>
              </div>
