          ...
          <div class="card mb-4">
            <div class="card-header pb-0">
              <div class="d-flex justify-content-between align-items-center">
                <div>
                  <h5>Form Tambah Mata Kuliah Syarat</h5>
                </div>
              </div>
            </div>
                    ...
                    <div class="form-group mb-4">
                      ...
                      <select name="id_matakuliah" id="id_matakuliah" class="form-control" required>
                        ...
                        <?php foreach (($matakuliah_list ?? []) as $m): ?>
                          <option value="<?= $m['id'] ?>"><?= esc($m['matakuliah']) ?> (<?= esc($m['kode']) ?>)</option>
                        <?php endforeach; ?>
                      </select>
                    </div>
                  </div>
                ...
                <div class="row">
                  <div class="col-md-12">
                    <div class="form-group mb-4">
                      ...
                      <input type="text" name="mk_syarat" id="mk_syarat" class="form-control <?= isset($validation) && $validation->hasError('mk_syarat') ? 'is-invalid' : '' ?>" placeholder="Masukkan nama mata kuliah syarat" value="<?= old('mk_syarat') ?>" required>
...
<body class="g-sidenav-show bg-primary">
  ...
  <script>
    document.getElementById('formMkSyarat').addEventListener('submit', function(e) {
